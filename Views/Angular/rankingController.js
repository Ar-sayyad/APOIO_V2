...
function getData() {
    ...
    /**** Search cell list or TUZ list : start ****/
    if (blockList === 'CELL') {
        var cellBatch = {
            "database": {
                "Method": "GET",
        ...
        var cellBatchStr = JSON.stringify(cellBatch, null, 2)
        var batchResult = processJsonContent(baseServiceUrl + "batch", 'POST', cellBatchStr);
        $.when(batchResult).fail(function () {
            warningmsg("Cannot Launch CELL Batch!!!");
        });
    } else {
        var tuzBatch = {
            "parent": {
